...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/beaches/beachesSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureBeachesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import CardBeaches from './CardBeaches';

const perPage = 10;

const TableSampleBeaches = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: beachesNotify,
    refetch,
  } = useAppSelector((state) => state.beaches);
  ...
  useEffect(() => {
    if (beachesNotify.showNotification) {
      notify(beachesNotify.typeNotification, beachesNotify.textNotification);
    }
  }, [beachesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/beaches/beaches-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {beaches && Array.isArray(beaches) && !showGrid && (
        <CardBeaches
          beaches={beaches}
          loading={loading}
...
export default TableSampleBeaches;
